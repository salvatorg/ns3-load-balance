...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < SERVER_COUNT; j++)
        {
            ...
            // NS_LOG_INFO ("Leaf - " << i << " is connected to Server - " << j << "(" << serverIndex << ") with address " << interfaceContainer.GetAddress(0) << " <-> " << interfaceContainer.GetAddress (1) << " with port " << netDeviceContainer.Get (0)->GetIfIndex () << " <-> " << netDeviceContainer.Get (1)->GetIfIndex ());
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < SPINE_COUNT; j++)
        {
			...
			for (int l = 0; l < LINK_COUNT; l++)
			{
				...
				NS_LOG_INFO (" Leaf ID :  " << leaves.Get (i)->GetId () << "," << i << "  Spine ID:  " << spines.Get (j)->GetId () << "," << j);
    ...
    if (runMode == TLB)
    {
        ...
        // Config::ConnectWithoutContext ("/NodeList/*/$ns3::Ipv4TLB/SelectPath",MakeCallback (&TLBPathSelectTrace));

        // Config::ConnectWithoutContext ("/NodeList/*/$ns3::Ipv4TLB/ChangePath",MakeCallback (&TLBPathChangeTrace));
...
//     }
    ...
    {
		// NS_LOG_INFO ( "RESULTS\n" );
		// NS_LOG_INFO ( "\tFalse Positives\t: " << CloveFalsePositives );
		// NS_LOG_INFO ( "\tFalse Negatives\t: " << CloveFalseNegatives );
		// NS_LOG_INFO ( "\tTrue Positives\t: " << CloveTruePositives );
		// NS_LOG_INFO ( "\tClove All\t: " << CloveAll );
		// NS_LOG_INFO ("RESULTS : " << CloveFalsePositives << " " << CloveFalseNegatives << " " << CloveTruePositives << " " << CloveAll);
		...
		for (int k = 0; k < SERVER_COUNT * LEAF_COUNT; k++)
		{
			...
			// for(std::map<std::pair<uint32_t, int>, std::pair<uint32_t, int> >::iterator iter1 = stat1.begin(); iter1 != stat1.end(); ++iter1)
			// {
			// 	NS_LOG_INFO ("FlowID: " << (iter1->first).first << " ackNum: " << (iter1->first).second << " GroupID: "<< (iter1->second).first << " ECN: " << (iter1->second).second);
			// 	stat2[std::make_pair ((iter1->first).first, (iter1->second).first)].unknown++;
			// }

			// for(std::map<std::pair<uint32_t, uint32_t>, GroupStats >::iterator iter2 = stat2.begin(); iter2 != stat2.end(); ++iter2)
			// {
			// 	NS_LOG_INFO ("FlowID: " << (iter2->first).first << " GroupID: " << (iter2->first).second << " ECNs: "<< (iter2->second).ECN << " noECNs: " << (iter2->second).noECN << " Unknown: " << (iter2->second).unknown );
			// }


 		}

		// NS_LOG_INFO ("\nRESULTS : " << TotalFalsePos << "/" << Total << " = " << (TotalFalsePos*100)/Total << "%\n");
