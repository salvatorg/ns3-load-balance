        ...
        long &flowCount, long &totalFlowSize, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME, uint32_t applicationPauseThresh, uint32_t applicationPauseTime)
{
    ...
    for (int i = 0; i < SERVER_COUNT; i++)//SERVER_COUNT
    {
        ...
        while (startTime < FLOW_LAUNCH_END_TIME)
        {
			...
			uint32_t	flowSize = (cdfTable!=NULL) ? gen_random_cdf (cdfTable) : 10000000;
...
int main (int argc, char *argv[])
{
    ...
    uint32_t cloveFlowletTimeout = 240;
    uint32_t cloveRunMode = 1;
    uint32_t cloveHalfRTT = 60;
    ...
    if (runMode == Clove)
    {
		...
		// Manual set up of forwarding table
		Ptr<Ipv4Clove> clove = servers.Get (0)->GetObject<Ipv4Clove> ();
		clove->AddAvailPath (1, 203);
		clove = servers.Get (1)->GetObject<Ipv4Clove> ();
		clove->AddAvailPath (1, 204);
	...
	install_single_application(0, 2, servers, 50000000, START_TIME+0.1, END_TIME);
	install_single_application(1, 2, servers, 50000000, START_TIME+0.1, END_TIME);
...
//     }
    ...
    if(cdfFileName.compare ("") != 0)free_cdf (cdfTable);
